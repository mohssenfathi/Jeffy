...
class GIFConverter {
    ...
    private(set) var gif: ConvertibleGIF
    ...
    init?(data : Data) {
        guard let gif = ConvertibleGIF(data: data) else { return nil }
        self.gif = gif
    }
...
class ConvertibleGIF {
    ...
    private func getGIFFrameDuration(imgSource: CGImageSource, index: Int) -> TimeInterval {
        ...
        guard let frameProperties = CGImageSourceCopyPropertiesAtIndex(imgSource, index, nil) else { return 0.02 }
        
        let fp = frameProperties as NSDictionary
        guard let gifProperties = fp[kCGImagePropertyGIFDictionary] as? NSDictionary,
